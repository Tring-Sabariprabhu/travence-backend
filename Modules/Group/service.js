...
export const groupList = async ({ user_id }) => {
    try {
        const { rows } = await db.query(`
                                SELECT g.*, u.email as created_user_email, u.name as created_user_name FROM groups g
                                INNER JOIN users u 
...
export const group = async ({ group_id }) => {
    try {
...
export const updateGroup = async ({ name, description, group_id }) => {
    try {
        const { rows } = await db.query(`
