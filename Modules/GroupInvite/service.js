...
import groupMemberResolvers from "../GroupMember/groupMemberResolvers.js";
import { setMailAndSend } from '../../helper/mailing/mailing.js';

export const getGroupInvitedList = async ({ admin_id }) => {
    try {
...
export const getGroupJoinRequestsForUser = async ({ email }) => {
    try {
...
export const resendGroupJoinRequests = async ({ admin_id, requestIDs }) => {
    try {
        if (requestIDs.length === 0) {
            throw new Error("No Requests found");
        }
        const adminInGroup = await groupMemberResolvers.Query.group_member(null, { member_id: admin_id });
...
export const deleteGroupJoinRequests = async ({ admin_id, requestIDs }) => {
    try {
        if (requestIDs.length === 0) {
            throw new Error("No Requests found");
        }
...
export const sendGroupJoinRequests = async ({ admin_id, emails }) => {
    try {
        if (emails.length === 0) {
...
export const acceptGroupJoinRequest = async ({ admin_id, user_id }) => {
    try {
        const adminInGroup = await groupMemberResolvers.Query.group_member(null, { member_id: admin_id });
        if (adminInGroup) {
            ...
            if (MemberExists[0]?.member_id === null || MemberExists[0]?.deleted_at !== null) {
                await groupMemberResolvers.Mutation.addUserToGroup(null, { group_id: adminInGroup?.group_id, user_id: user_id });
...
export const declineGroupJoinRequest = async ({ request_id }) => {
    try {
