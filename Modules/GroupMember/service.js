...
export const group_member = async ({ member_id }) => {
    try {
...
export const group_members = async ({ group_id }) => {
    try {
        const { rows: members } = await db.query(`
                            SELECT gm.* FROM group_members gm 
                            INNER JOIN users u 
                                ON gm.user_id = u.user_id 
                            WHERE gm.group_id = $1 AND gm.deleted_at IS NULL AND u.deleted_at IS NULL ORDER BY gm.role DESC `, [group_id]);
        return members;
    }
    catch (err) {
        console.log(err.message);
...
export const addUserToGroup = async ({ group_id, user_id, role = "member" }) => {
    try {
...
export const deleteUserFromGroup = async ({ admin_id, member_id }) => {
    try {
        ...
        const user = await groupMemberResolvers.Query.group_member(null, { member_id: member_id });
        if (user) {
...
export const changeRoleInGroup = async({ admin_id, member_id })=>{
    try {
        const adminInGroup = await groupMemberResolvers.Query.group_member(null, { member_id: admin_id });
        if (!adminInGroup || adminInGroup?.deleted_at !== null) {
            throw new Error("Access denied! You are Removed From this Group");
